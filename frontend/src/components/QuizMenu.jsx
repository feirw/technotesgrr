...
const QuizMenu = ({ onSelect, onClose, categoryAnswers = {} }) => {
  ...
  const filtered = quizzes.filter((x) => !q.trim() || x.title.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div ref={dialogRef} className="relative rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden bg-gradient-to-br from-white to-pink-50" style={{ border: `3px solid ${BRAND}`, maxHeight: '85vh' }} initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 50 }} transition={{ type: 'spring', stiffness: 300 }}>
          
          {/* Header */}
          <div className="sticky top-0 z-10 bg-gradient-to-r from-pink-500 to-rose-500 text-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <motion.h3 className="text-3xl font-black" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                📚 Επιλογή Κεφαλαίου
              </motion.h3>
              <motion.button onClick={onClose} className="p-2 rounded-full hover:bg-white/20 transition-colors" whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }}>
                <X className="w-6 h-6" />
              </motion.button>
            </div>
            ...
            <div className="relative">
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Αναζήτηση..." className="w-full rounded-xl px-4 py-3 pl-11 pr-10 text-gray-800 outline-none" />
          ...
          <div className="overflow-y-auto p-6" style={{ maxHeight: 'calc(85vh - 160px)' }}>
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtered.map((quiz, i) => (
                  <motion.button key={quiz.id} onClick={() => onSelect?.(quiz)} className="group relative p-6 rounded-2xl bg-white border-2 border-pink-200 hover:border-pink-400 hover:shadow-2xl transition-all text-left overflow-hidden" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileHover={{ y: -6, scale: 1.03 }} whileTap={{ scale: 0.98 }}>
                    
                    {/* Progress Bar */}
                    <div className="absolute top-0 left-0 h-1.5 w-full bg-gray-200">
                      <motion.div className="h-full bg-gradient-to-r from-pink-500 to-rose-500" initial={{ width: 0 }} animate={{ width: `${quiz.percent}%` }} transition={{ duration: 1, delay: i * 0.05 + 0.3 }} />
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      {quiz.percent === 100 && (
                        <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 500, delay: i * 0.05 + 0.5 }}>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">{quiz.answered}/{quiz.total} ερωτήσεις</span>
                      <span className="font-black text-xl" style={{ color: quiz.percent === 100 ? '#10b981' : BRAND }}>
                        {quiz.percent}%
                      </span>
                    </div>
                  </motion.button>
