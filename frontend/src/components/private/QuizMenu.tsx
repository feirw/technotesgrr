...
import { fetchAllQuizzes, QuizData } from '../../utils/quizUtils';

// --- Types & Interfaces ---

// Represents the value stored in local storage/state for a specific question
// Based on the usage in the JS file (ans.selectedAnswer, ans.timestamp)
interface AnswerEntry {
  selectedAnswer: number;
  timestamp?: number;
}

// Maps Question Index -> AnswerEntry
type QuizAnswers = Record<string, AnswerEntry | number>;

// Maps Quiz ID -> QuizAnswers
type CategoryAnswers = Record<string, QuizAnswers>;
...
interface QuizMenuProps {
  onSelect: (quiz: ProcessedQuiz, startIdx: number) => void;
  onClose: () => void;
...
const QuizMenu: React.FC<QuizMenuProps> = ({ onSelect, onClose, categoryAnswers = {} }) => {
  ...
  const [sortBy, setSortBy] = useState<'default' | 'progress' | 'title' | 'recent'>('default');
  ...
  const processedQuizzes = useMemo(() => {
    ...
    if (sortBy === 'progress') {
      ...
      result.sort((a, b) => {
        const getLastTimestamp = (quizId: string) => {
          const answers = categoryAnswers[quizId] || {};
          const timestamps = Object.values(answers).map((ans) => {
            // Handle if answer is just a number (no timestamp)
            if (typeof ans === 'number') return 0;
            return ans.timestamp ? new Date(ans.timestamp).getTime() : 0;
          });
