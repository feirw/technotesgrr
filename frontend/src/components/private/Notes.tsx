...
const Palia: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<number | null>(null);
  const [selectedSubExam, setSelectedSubExam] = useState<number | null>(null);
  ...
  const getSubExamsForCategory = (categoryId: number): Exam[] => {
  ...
  const handleOpenNew = () => {
    if (currentPdf) {
      window.open(currentPdf.pdfPath, '_blank', 'noopener,noreferrer');
    }
  };
  ...
  const handleSubExamClick = (examId: number) => {
