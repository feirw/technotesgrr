...
export default function ChatWidget({ nickname }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "bot", content: "Ωχ! Κάτι πήγε στραβά. Δοκίμασε ξανά σε λίγο.", ts: Date.now() }]);
    } finally {
      setSending(false);
    }
  };
    ...
    <>
      ...
      {open && (
        <div
          ...
          role="dialog" aria-label="Chat technotesgr"
        >
          {/* Header */}
          <div className="px-4 py-3 bg-gradient-to-r from-[#ffd1d1] to-white dark:from-[#222] dark:to-[#111] flex items-center justify-between">
            ...
            <button onClick={() => setOpen(false)} className="text-sm opacity-70 hover:opacity-100">Κλείσιμο</button>
          </div>
