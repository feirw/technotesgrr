...
import { Code, Briefcase, TrendingUp, DollarSign, Globe, Shield, Home, Palette, CheckCircle, GraduationCap } from 'lucide-react';

/**
 * Δεδομένα Κουίζ για το 4ο Επιστημονικό Πεδίο & Συναφείς Σχολές
 */

// 1. Ερωτήσεις (Q_ID: Ερώτηση) - 8 Κατηγορίες: INFO, FIN, DIOIK, OIK, SERV, PEDAGOGIKA, SOMATA, TEXNES
const QUESTIONS = {
...
// 2. Πίνακας Βαθμολόγησης (Συντελεστές) - 8 Κατηγορίες: INFO, FIN, DIOIK, OIK, SERV, PEDAGOGIKA, SOMATA, TEXNES
const SCORE_MATRIX = {
    // INFO, FIN, DIOIK, OIK, SERV, PEDAGOGIKA (P), SOMATA (S), TEXNES (T)
    1: [4, 1, 1, 0, 0, 0, 0, 0], 2: [3, 1, 1, 0, 0, 0, 0, 0], 3: [4, 1, 0, 1, 0, 0, 0, 0],
    4: [0, 1, 0, 4, 1, 0, 0, 0], 5: [0, 1, 0, 3, 0, 0, 0, 0], 6: [0, 0, 0, 4, 0, 0, 0, 0],
    7: [1, 1, 4, 0, 0, 0, 0, 0], 8: [0, 0, 4, 0, 1, 0, 0, 0], 9: [0, 1, 3, 0, 0, 0, 0, 0],
    10: [0, 4, 0, 0, 0, 0, 0, 0], 11: [0, 4, 0, 1, 0, 0, 0, 0], 12: [1, 3, 0, 1, 0, 0, 0, 0],
    13: [0, 0, 0, 1, 4, 0, 0, 0], 14: [0, 0, 1, 0, 3, 0, 0, 0], 15: [0, 0, 0, 0, 4, 0, 0, 0],
    16: [0, 0, 0, 0, 0, 0, 3, 0], 17: [0, 0, 0, 0, 0, 0, 4, 0], 18: [0, 0, 0, 0, 0, 0, 0, 2],
    19: [3, 1, 1, 0, 0, 0, 0, 0], 20: [0, 0, 0, 0, 0, 4, 0, 0], 21: [0, 0, 0, 0, 0, 4, 0, 0],
    22: [0, 0, 0, 0, 0, 3, 0, 0], 23: [0, 0, 0, 0, 0, 5, 0, 0], 24: [0, 0, 0, 0, 0, 0, 0, 4],
    25: [0, 0, 0, 0, 0, 0, 0, 4], 26: [0, 0, 0, 0, 0, 0, 0, 5], 27: [0, 0, 0, 0, 0, 0, 0, 3],
    28: [2, 2, 2, 1, 2, 0, 1, 1], 29: [0, 1, 1, 1, 1, 1, 1, 1], 30: [3, 0, 1, 1, 0, 0, 0, 0],
    31: [0, 4, 0, 0, 0, 0, 0, 0], 32: [0, 0, 3, 0, 1, 1, 0, 0], 33: [3, 1, 1, 0, 0, 0, 0, 0],
    34: [0, 0, 0, 1, 0, 0, 0, 0], 35: [0, 4, 0, 0, 0, 0, 0, 0], 36: [1, 2, 0, 1, 0, 1, 0, 0],
    37: [0, 0, 0, 0, 0, 4, 0, 0], 38: [0, 0, 0, 0, 0, 0, 0, 3], 39: [0, 0, 0, 0, 0, 0, 4, 0],
    40: [0, 0, 0, 0, 3, 0, 0, 0], 41: [0, 0, 0, 0, 1, 1, 0, 0], 42: [0, 0, 0, 0, 0, 0, 0, 3],
    43: [1, 1, 1, 1, 1, 0, 2, 0], 44: [0, 0, 0, 3, 0, 0, 0, 0], 45: [0, 0, 0, 3, 0, 1, 1, 0],
};

// 3. Κατηγορίες και Πληροφορίες Αποτελεσμάτων (με βάση τα στοιχεία 2025)
const CATEGORY_NAMES = ['INFO', 'FIN', 'DIOIK', 'OIK', 'SERV', 'PEDAGOGIKA', 'SOMATA', 'TEXNES'];
const RECALCULATED_MAX_SCORES = {
    INFO: 170, FIN: 155, DIOIK: 140, OIK: 125, SERV: 110, PEDAGOGIKA: 80, SOMATA: 80, TEXNES: 95
};


const RESULTS_MAPPING = {
    INFO: {
        title: 'Πληροφορική & Τεχνολογική Διοίκηση 🚀',
        description: 'Η κλίση σας είναι στον συνδυασμό της τεχνολογίας, των συστημάτων και της διοίκησης. Σας ταιριάζουν σχολές που εστιάζουν στην ανάλυση δεδομένων, τον ψηφιακό μετασχηματισμό και τη λήψη αποφάσεων με τεχνολογικά εργαλεία.',
        icon: Code,
    ...
    FIN: {
        ...
        description: 'Είστε ακριβής, αναλυτικός/ή και σας ελκύει ο κόσμος των αριθμών, των επενδύσεων και της διαχείρισης κινδύνου. Η καριέρα σας βρίσκεται στις τράπεζες, τα λογιστικά γραφεία και τις ασφαλιστικές εταιρείες.',
        icon: DollarSign,
        color: 'text-pink-600',
    ...
    DIOIK: {
        ...
        description: 'Έχετε κλίση στην οργάνωση, τη διοίκηση, τη στρατηγική και την επικοινωνία. Σας ταιριάζουν ρόλοι που απαιτούν ηγετικές ικανότητες, διαπραγμάτευση και εστίαση στην ανάπτυξη της επιχείρησης και του προϊόντος.',
        icon: Briefcase,
        color: 'text-rose-600',
        ...
        schools: [
            ...
            { name: 'Διοίκησης Επιχειρήσεων και Οργανισμών (ΕΚΠΑ)', city: 'Αθήνα', code: 1005, base: '16.220' },
        ],
    },
    OIK: {
        title: 'Θεωρητικά & Ακαδημαϊκά Οικονομικά 🎓',
        description: 'Το ενδιαφέρον σας εστιάζεται στην ακαδημαϊκή ανάλυση, τη θεωρία και την εις βάθος κατανόηση του οικονομικού συστήματος. Σας ταιριάζουν ρόλοι που απαιτούν υψηλή μαθηματική/αναλυτική σκέψη για μοντελοποίηση.',
        icon: TrendingUp,
        color: 'text-red-500',
    ...
    SERV: {
        ...
        description: 'Σας ελκύει το διεθνές περιβάλλον, οι διεθνείς σχέσεις και οι κλάδοι των υπηρεσιών, ειδικά ο Τουρισμός και η Ναυτιλία. Οι κορυφαίες επιλογές απαιτούν **Ειδικό Μάθημα Ξένης Γλώσσας**.',
        icon: Globe,
    ...
    PEDAGOGIKA: {
        ...
        description: 'Η κλίση σας είναι στην εκπαίδευση, τη φροντίδα και τη στήριξη των μαθητών. Σας ταιριάζουν σχολές που οδηγούν σε διδακτικούς ρόλους στην Πρωτοβάθμια Εκπαίδευση (Δάσκαλοι, Νηπιαγωγοί) ή την Ειδική Αγωγή.',
        icon: GraduationCap,
        color: 'text-pink-400',
        ...
        schools: [
            ...
            { name: 'Εκπαίδευσης και Αγωγής στην Προσχολική Ηλικία (ΕΚΠΑ)', city: 'Αθήνα', code: 154, base: '14.800' },
        ],
    },
    SOMATA: {
        title: 'Σώματα Ασφαλείας & Φυσική Αγωγή 🛡️',
        description: 'Έχετε ισχυρή κλίση προς τη δράση, την πειθαρχία και την προσφορά μέσα από Σώματα Ασφαλείας ή τον αθλητικό χώρο. ΠΡΟΣΟΧΗ: Αυτές οι σχολές απαιτούν **Αγωνίσματα** και **Υγειονομικές Εξετάσεις**.',
        icon: Shield,
        color: 'text-rose-400',
        gradient: 'from-rose-50 to-white dark:from-rose-800/40 dark:to-gray-900',
        schools: [
            { name: 'ΣΣΑΣ Πληροφορικής / Οικονομικό', city: 'Θεσσαλονίκη', code: 889/867, base: '18.240 - 17.735', note: 'Αγων./Υγειονομικά' },
    ...
    TEXNES: {
        ...
        description: 'Η δημιουργικότητα και η καλλιτεχνική σας έκφραση είναι υψηλές προτεραιότητες. Σας ταιριάζουν σχολές που απαιτούν ταλέντο και εξέταση σε **Ειδικά Μαθήματα** (Σχέδιο, Μουσική).',
        icon: Palette,
...
const Prosanatolismospage = () => {
    ...
    const calculateResults = () => {
        ...
        if (Object.keys(answers).length !== allQuestions.length || allQuestions.some(qId => answers[qId] === null || isNaN(answers[qId]))) {
        ...
        const initialScores = { INFO: 0, FIN: 0, DIOIK: 0, OIK: 0, SERV: 0, PEDAGOGIKA: 0, SOMATA: 0, TEXNES: 0 };
        const finalScores = allQuestions.reduce((acc, qId) => {
            const score = answers[qId];
            const weights = SCORE_MATRIX[qId];
        ...
        const scoresArray = Object.entries(finalScores);
        ...
        const tiedCategories = scoresArray.filter(([, score]) => score === maxScore).map(([category]) => category);
    ...
    const ResultCard = ({ item }) => {
        ...
        const currentScore = results.finalScores[category];
        const maxPossibleScore = RECALCULATED_MAX_SCORES[category];
        const percentage = Math.min(100, Math.round((currentScore / maxPossibleScore) * 100)); // Limit to 100%

        return (
            <div className={`p-6 border-t-8 border-${data.color.split('-')[1]}-600 rounded-lg shadow-2xl bg-gradient-to-br ${data.gradient} transition transform hover:scale-[1.02] duration-300`}>
                ...
                <div className="mb-4">
                    <div className="w-full bg-gray-300 rounded-full h-3 dark:bg-gray-700 shadow-inner">
                        ...
                        <div className={`h-3 rounded-full bg-gradient-to-r from-pink-400 to-rose-600`} style={{ width: `${percentage}%` }}></div>
                    </div>
                ...
                <div className="mt-4 border-t pt-4 border-gray-200 dark:border-gray-700">
                    <p className="font-extrabold text-lg text-gray-900 dark:text-white mb-2">Κορυφαίες Προτάσεις Σχολών:</p>
                    <ul className="space-y-3">
                        {data.schools.map((school, index) => (
                            <li key={index} className="flex items-start text-sm bg-white dark:bg-gray-800 p-3 rounded-md shadow-inner">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600 dark:text-gray-400">
                                        Κωδ.: {school.code} | Βάση 2025: {school.base} Μόρια {school.note ? `(${school.note})` : ''}
                                    </p>
                                </div>
                            </li>
                        ))}
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 bg-white dark:bg-gray-900 min-h-screen">
            ...
            {results && (
                <div className="mb-12 p-8 rounded-2xl bg-rose-50 dark:bg-gray-800 border border-rose-200 dark:border-rose-800 shadow-2xl"> {/* Rose Background */}
                    <h2 className="text-3xl font-bold text-rose-700 dark:text-rose-300 mb-5 text-center"> {/* Rose Header */}
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {results.sortedScores.slice(0, Math.max(results.tiedCategories.length, 3)).map((item) => (
                            <ResultCard key={item.category} item={item} />
                        ))}
                    </div>
                </div>
            )}
                ...
                <div className="overflow-x-auto bg-gray-100 dark:bg-gray-800 p-6 rounded-xl shadow-inner">
                    <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-600">
                        <thead className="bg-gray-200 dark:bg-gray-700">
                            <tr>
                                <th className="px-4 py-3 text-left text-sm font-extrabold text-gray-700 uppercase tracking-wider dark:text-gray-300">Κωδ.</th>
                                <th className="px-6 py-3 text-left text-sm font-extrabold text-gray-700 uppercase tracking-wider dark:text-gray-300">Κατεύθυνση</th>
                                <th className="px-6 py-3 text-left text-sm font-extrabold text-gray-700 uppercase tracking-wider dark:text-gray-300">Προσανατολισμός Μαθημάτων</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-800 dark:text-gray-200">
                            {CATEGORY_NAMES.map(name => (
                                <tr key={name} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                    <td className="px-4 py-4 whitespace-nowrap font-extrabold">{name}</td>
                                    <td className="px-6 py-4 whitespace-normal font-semibold">{RESULTS_MAPPING[name].title}</td>
                                    <td className="px-6 py-4 whitespace-normal text-sm">
                                        {name === 'INFO' && 'Υψηλή βαρύτητα στην Πληροφορική, Μαθηματικά & Διοίκηση (ΔΕΤ).'}
                                        {name === 'FIN' && 'Απαιτεί μεγάλη ακρίβεια, Μαθηματική ανάλυση και γνώσεις Λογιστικής & Χρηματοοικονομικής.'}
                                        {name === 'DIOIK' && 'Εστιάζει στη Διοίκηση Ανθρώπων, Marketing και Στρατηγική Επιχειρήσεων.'}
                                        {name === 'OIK' && 'Έμφαση στη Θεωρία και την Ακαδημαϊκή/Ερευνητική Κατανόηση των Οικονομικών Φαινομένων.'}
                                        {name === 'SERV' && 'Πολύ υψηλές βάσεις. Απαιτείται συνήθως Ειδικό Μάθημα Ξένης Γλώσσας.'}
                                        {name === 'PEDAGOGIKA' && 'Εκπαίδευση, φροντίδα και καθοδήγηση. Καλύπτει Δημοτική/Προσχολική Εκπαίδευση.'}
                                        {name === 'SOMATA' && 'Δράση, πειθαρχία, φυσική ετοιμότητα. Απαιτούνται Αγωνίσματα και Υγειονομικά.'}
                                        {name === 'TEXNES' && 'Καλλιτεχνική έκφραση και Design. Απαιτούνται Ειδικά Μαθήματα (Μουσική/Σχέδιο).'}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
...
// Βοηθητικό Component για την εμφάνιση κάθε ερώτησης
const QuestionBlock = ({ qId, question, selectedScore, onChange }) => {
    ...
    return (
        <div className="mb-6 p-5 border border-gray-300 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800 shadow-md hover:shadow-lg transition duration-300">
            <label className="block text-gray-800 dark:text-gray-200 font-semibold mb-3 text-md">
                <span className="text-rose-600 dark:text-rose-400 font-extrabold mr-2">{qId}.</span> {question} {/* Rose Accent */}
            </label>
            <div className="flex justify-between space-x-2 bg-white dark:bg-gray-900 p-2 rounded-lg shadow-inner">
                {scoreOptions.map((score) => (
                    <div key={score} className="flex flex-col items-center cursor-pointer transition duration-150 ease-in-out hover:bg-gray-100 dark:hover:bg-gray-700 p-1 rounded-md" onClick={() => onChange(qId, score)}>
                        <input
                            type="radio"
                            id={`q${qId}-${score}`}
                            name={`question-${qId}`}
                            ...
                            className="form-radio h-5 w-5 text-rose-600 dark:bg-gray-700 dark:border-gray-600 focus:ring-rose-500" // Rose Radio Button
                        />
                        <label
                            htmlFor={`q${qId}-${score}`}
                            className={`text-xs font-bold mt-1 ${selectedScore === score ? 'text-rose-600 dark:text-rose-400' : 'text-gray-500 dark:text-gray-400'}`} // Rose Selected Text
                        >
                            {score}
                        </label>
                    </div>
